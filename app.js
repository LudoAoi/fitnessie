...
function handleOnboardingSubmit(e) {
    ...
    userData.trainingIntensity = formData.get('trainingIntensity');

    Object.keys(userData).forEach(key => {
        if (['age', 'weight', 'height', 'fat_percent', 'muscle_percent', 'training_days', 'max_squats', 'max_pushups', 'max_pullups'].includes(key)) {
...
function generateWeeklyBasket(userProfile) {
    const { diet_preference, allergies = [] } = userProfile;
    
    let availableRecipes = recipeLibrary.filter(recipe => {
        const hasDietTag = recipe.diet_tags.includes(diet_preference);
        const hasNoAllergens = !recipe.allergy_tags.some(tag => allergies.includes(tag));
...
function generateDynamicMealPlan(userData, weeklyIngredientBasket, checkinData = {}) {
    const { weight, height, age, sex, goal, training_days, diet_preference, fat_percent, allergies = [] } = userData;
    ...
    if (checkinData.hunger_level && goal === 'lose_fat' && checkinData.hunger_level >= 4) { targetCalories += 100; }
    ...
    let baseCandidatePool = recipeLibrary.filter(recipe => {
        const hasDietTag = recipe.diet_tags.includes(diet_preference);
        const hasNoAllergens = !recipe.allergy_tags.some(tag => allergies.includes(tag));
        return hasDietTag && hasNoAllergens;
    });
...
function getWorkoutForDay(day, dayIndex, plan, deloadExercises = [], focusMuscles = []) {
    ...
    const { training_days, goal, trainingIntensity } = userProfile;
    ...
    return { title: dayWorkout.title, sessions };
}

function getBestExerciseFor(muscleGroup, userData, isAccessory = false, isFocus = false) {
    const { equipment, limitations, training_environment, goal, trainingIntensity } = userData;
    ...
    if (limitations && limitations.length > 0) {
        limitations.forEach(limitation => {
            const contraindicatedIds = contraindications[limitation] || [];
            allCandidates = allCandidates.filter(ex => !contraindicatedIds.includes(ex.id));
        });
    }
...
function renderDay(day) {
    ...
    const sessionsHtml = dayData.sessions.map((s, sIdx) => {
        ...
        return `<div class="card"><h3 class="text-2xl font-bold mb-2">${s.name}</h3><div class="space-y-2">${exercisesHtml}</div></div>`;
    }).join('');
...
function handleOnboardingNav(direction) {
    ...
    if (direction === 'next') {
        ...
        if (currentOnboardingStep === 5) { // Equipment validation
    ...
    const stepInfo = {
        ...
        3: { title: "How do you like to eat?", subtitle: "Select your dietary preferences and any allergies." },
        4: { title: "Where do you train?", subtitle: "This helps us select the right exercises for you." },
        5: { title: "What's in your gym bag?", subtitle: "Select all the equipment you have access to." },
        6: { title: "What's your training intensity?", subtitle: "This helps us tailor the workout volume to your goals." },
        7: { title: "Any pain or limitations?", subtitle: "This helps us select safe and effective exercises for you." },
        8: { title: "How do you like to train?", subtitle: "This helps us gauge your current fitness level." },
        9: { title: "When should we begin?", subtitle: "Choose your first day and let's get started!" }
    };
...
function generateShoppingListCSV(peopleCount) {
    ...
    Object.values(weeklyIngredients).forEach(ing => {
        const scaledQuantity = Math.ceil((ing.quantity * peopleCount) * 10) / 10; // Scale and round up to one decimal
